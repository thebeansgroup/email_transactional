...
def directory_builder
  ...
  Dir.glob(build_dir + '/emails/*/').each do |folder|
    html += '<li><a href='
    # {folder.sub('build/', '')}'>#{folder.sub('build/emails', '')}</a></li>'
    # sub
  ...
  def initialize(app, options_hash = {}, &block)
    ...
    app.after_build do |builder|
      Dir.glob(build_dir + File::SEPARATOR + '**/*.html').each do |source_file|
        ...
        # premailer = Premailer.new(source_file,
        # verbose: true, css: 'http://localhost:4567/stylesheets/all.css',
        ...
        File.open(destination_txt_file, 'w') do |content|
          content.puts(parse_text(premailer.to_plain_text))
        end
